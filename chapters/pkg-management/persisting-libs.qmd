...
When working within a Databricks notebook `.libPaths()` typically returns multiple values. Exact paths and ordering may vary by Databricks Runtime version and workspace configuration, but often look like:
...
Given that `.libPaths()` can return multiple values in a notebook you might wonder if there a "best" position to add your new volume path(s) to, that will depend on how you want packages to behave.

A safe default is to add a path *after* the cluster libraries location, this will make it appear as if the Databricks Runtime has been extended to include packages in the volume path(s).
...
`R_LIBS_SITE` can be set via an [environment variable](https://docs.databricks.com/en/compute/configure.html#environment-variables) but is referenced by `/etc/R/Renviron.site` and provides limited control over where the path will appear in the `.libPaths()` order. For deterministic ordering, use an init script to alter `/etc/R/Renviron.site` directly.
